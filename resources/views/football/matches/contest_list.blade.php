...
@section('contents')
@php
    $localTeam = collect($match->participants)->firstWhere('location', 'home');
    $visitorTeam = collect($match->participants)->firstWhere('location', 'away');
@endphp
    <div class="row mb-2">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-end row">
                        <div class="col-sm-12">
                            <div class="card-body">
                                <h5 class="card-title text-primary">{{ $match->name }} ( {{ $match->league->name }})</h5>
                                <div class="container mt-4">
                                    <div class="row  text-center">
                                        <div class="col-5 text-start">
                                            <img src="{{ getsportmonksImage($localTeam->image_path) }}" height="50"
                                                width="50" alt="Local Team" class="img-fluid rounded">
                                            <p class="mt-2 fw-bold">{{ $localTeam->short_code ?? $localTeam->name }}</p>
                                        </div>
                                        <div class="col-2 text-center">
                                            <p class="h4 fw-bold">VS</p>
                                        </div>
                                        <div class="col-5 text-end">
                                            <img src="{{ getsportmonksImage($visitorTeam->image_path) }}"
                                                height="50" width="50" alt="Visitor Team" class="img-fluid rounded">
                                            <p class="mt-2 fw-bold">{{ $visitorTeam->short_code ?? $visitorTeam->name }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        @foreach ($moreContests as $contest)
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="contest_{{ $contest->id }}">
                                    First Prize: {{ $contest->first_prize }} {{ $contest->contest_type }}
                                    (&#8377; {{ $contest->entry_fees }})
                                </label>
                            </div>
                        @endforeach
    ...
    <div class="row mb-2">
        <divl class="col-sm-12">
           @livewire('football.match.contestList',['fixture'=>$match])
        </divl>
    </div>
@endsection
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#addContestBtn').on('click', function() {
                ...
                let url = "{{ route('admin.football.match.contests.add', $match->match_id) }}";
