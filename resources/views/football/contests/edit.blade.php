...
@section('contents')
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="card-title m-0 me-2">Edit Contest</h5>
            <a href="{{ route('admin.football.default.contest.index') }}" class="btn btn-danger">Back</a>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.football.default.contest.edit', $contest->id) }}" method="POST">
                @csrf
                ...
                <div class="row mb-2">
                        ...
                        <select class="form-select @error('contest_type') is-invalid @enderror" id="contest_type_select"
                            ...
                            @foreach ($contest_types as $data)
                                <option value="{{ $data->id }}"
                                    {{ old('contest_type', $contest->contest_type_id) == $data->id ? 'selected' : '' }}>
                                    {{ $data->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-sm-6">
                        ...
                        <input type="number" min="1" class="form-control @error('max_entries') is-invalid @enderror"
                            id="max_entries" name="max_entries" value="{{ old('max_entries', $contest->max_entries) }}"
                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        ...
                        <input type="number" min="0" class="form-control @error('total_bots') is-invalid @enderror"
                            id="total_bots" name="total_bots" value="{{ old('total_bots', $contest->bot_user) }}"
                            placeholder="Total Bots">
                        @error('total_bots')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row my-2">
                    ...
                    <div class="col-sm-3">
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" id="is_bonus_contest"
                                name="is_bonus_contest"
                                {{ old('is_bonus_contest', $contest->is_bonus_contest) ? 'checked' : '' }}>
                    ...
                    @elseif(isset($contest->prizeBreakup))
                        @foreach ($contest->prizeBreakup as $key => $prize)
                            <div class="row price-row">
                                <div class="col-4">
                                    <label class="form-label">From Rank</label>
                                    <input type="number" min="1" class="form-control" name="from_rank[]"
                                        value="{{ $prize->rank_from }}" placeholder="From Rank">
                                </div>
                                <div class="col-4">
                                    <label class="form-label">To Rank</label>
                                    <input type="number" min="1" class="form-control" name="to_rank[]"
                                        value="{{ $prize->rank_upto }}" placeholder="To Rank">
                                </div>
                                <div class="col-4">
                                    <label class="form-label">Prize</label>
                                    <input type="number" min="1" class="form-control" name="prize[]"
